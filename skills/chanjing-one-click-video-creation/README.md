...
| **`scripts/run_render.py`** | **确定性成片**：读 `workflow.json`，调用当前 skill 内 `clients.tts_client`、`clients.avatar_client`、`clients.ai_creation_client`，写 `work/` 与 `final_one_click.mp4`；行为须符合 **`render_rules.md`**、**`SKILL.md` §5**。 |
| **`examples/workflow-input.example.json`** | `run_render` 输入结构示例，字段含义对照 `SKILL.md` §5（输入 MVP）。 |
| **`tests/`** | 桩测试、测试用 ID 解析与本地测试默认值模板；当前测试链路使用本 skill 自己的 `clients/` 能力，不再依赖外部 skill 脚本。 |
...
| **templates/visual_prompt_people_constraint.md** | 文生视频人物与族裔：**触发条件**下须在 `ref_prompt` 显式写入族裔锚定；含身份动作刻画与推荐英文短语。 |
...
2. **渲染层**：对已定稿口播做 **TTS**（过长则按分镜合并少批次）→ **按镜切音频** → 数字人镜走 **音频驱动数字人视频合成**，AI 镜走 **文生视频 + 与镜内音频合成** → 按公共数字人轨统一分辨率/帧率等 → **ffmpeg 顺序拼接** → 输出 **`final_one_click.mp4`** 与 **`workflow_result.json`**。细则见 **`templates/render_rules.md`**。

既可由 Agent **分步**产出 `video_plan` / `full_script` / `scenes[]`，也可在已有 `workflow.json` 时**只跑** `run_render.py`（见下节「环境与运行」）。
...
flowchart TD
  ...
  TTS --> CUT[按镜切音频]
  CUT --> BR{按镜类型}
  BR -->|数字人镜| DH[avatar_client]
  BR -->|AI 镜| AI[ai_creation_client 文生视频]
  AI --> MUX[镜内音视频合成]
  DH --> NORM[统一封装参数]
  MUX --> NORM
  NORM --> FF[ffmpeg 拼接]
...
`run_render.py` **不会**从环境变量读取默认音色、数字人 `person_id` 或 `figure_type`。每次成片须在 **`workflow.json` 根级**写明 **`audio_man`**、存在数字人镜时的 **`person_id`**（或 `avatar_id`）与 **`figure_type`**。请按当次 **`video_plan`** 与口播人设，调用当前 skill 内的音色与数字人查询能力，选择合适的返回 ID 后填入；勿依赖 shell `export` 或仓库内跨任务的隐式「默认」记录。

当前主渲染链已内聚实现：

- TTS：`clients.tts_client`
- 数字人视频：`clients.avatar_client`
- AI 文生视频：`clients.ai_creation_client`
