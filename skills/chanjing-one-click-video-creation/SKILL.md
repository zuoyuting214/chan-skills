...
- **本机**：`ffmpeg`、`ffprobe`  
- **`run_render` / 仓库根**：未设置 **`CHAN_SKILLS_DIR`** 时，脚本按自身路径推断：自 `skills/chanjing-one-click-video/scripts/run_render.py` **向上四级**为含 `skills/chanjing-tts` 等的仓库根。仅当目录布局不是「仓库内标准 skill 路径」或单独拷贝本 skill 时，才需显式设置 **`CHAN_SKILLS_DIR`**  
